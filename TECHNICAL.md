...
After processing, the mobile cache copies the authoritative server `remote_status`. A `review_required` result exposes Review & Finalize. Tapping Finalize queues a persistent server job and displays `Finalizing`; mobile changes to `finalized` only after the authenticated API confirms the reviewed record and its embeddings were committed. If finalization is queued while processing sync is still active, the worker automatically runs another pass for the newly queued job. A session with `delete_after_review` deletes its physical local recording after review. Deleting a session also completes remote deletion and physical local-audio deletion before removing the SQLite session row.
