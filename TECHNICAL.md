...
2. **Evidence distillation:** the configured language-model provider isolates exercise names, approximate time ranges, completion state, sets, reps, weights, equipment, personalized cues, user observations, pain observations, and progression suggestions into a compact evidence timeline. Sessions use approximately 15-minute windows, and all window calls run in parallel.
...
4. **Whole-session synthesis:** one additional language-model call receives only the compact distilled transcript, not the raw transcript. It reconciles credible boundary duplicates and produces the existing `ExtractionOutput`, including the exercise records and transcript-supported session themes, accomplishments, improvement areas, pain observations, and next-session plan.
5. **Exercise canonicalization and media matching:** no LLM call. Synthesized names are deterministically matched against a commit-pinned public GitHub exercise dataset. The dataset begins loading in parallel with model processing; only the compact matching fields are retained in the shared Next.js data cache and warm-instance memory. Low-confidence or ambiguous matches preserve the model’s original name. When `EXERCISE_MEDIA_BASE_URL` is configured, confident matches also receive structured `referenceMedia` metadata resolved against that licensed HTTPS host. Dataset failures are non-fatal.
6. **Summary rendering:** no additional LLM call. `generateSummaryText()` deterministically formats the synthesized structured record into the compact workout recap, keeping display output stable and ensuring later user edits can be rendered without another model call.
7. **Review and indexing:** the initial extraction is stored as `review_required` but is not indexed for Ask AI. Finalization saves the user-reviewed exercises and batches the resulting overview, complete exercise-set, and next-plan chunks in one Voyage embeddings request. The finalized status update and chunk replacement commit in one Postgres transaction after embeddings are ready.

Model output is parsed as untrusted JSON. The distillation stage normalizes optional arrays, numbers, statuses, and malformed evidence entries. The synthesis stage requires a valid object, exercise array, exercise names, and set arrays before the result can enter canonicalization or persistence. Invalid output fails safely and leaves the session eligible for retry rather than persisting a partial recap.
