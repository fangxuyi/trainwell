...
2. **Evidence distillation:** the configured language-model provider isolates spoken exercise names, neutral movement descriptions, transcript-supported body regions and category, approximate time ranges, completion state, sets, reps, weights, equipment, personalized cues, user observations, pain observations, and progression suggestions into a compact evidence timeline. Sessions use approximately 15-minute windows, and all window calls run in parallel.
