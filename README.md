...
The Build Week deployment uses the OpenAI provider implemented in `apps/api/lib/language-model.ts`, which defaults to `gpt-5.6-terra`. GPT-5.6 first isolates exercise evidence into a compact timeline and classifies each exercise as performed, actively instructed, planned for later, merely mentioned, or unclear. Conversational mentions are excluded from the workout record. Motion Memo then deterministically retrieves likely exercises from its pinned reference dataset using spoken names, movement descriptions, equipment, body regions, and category—never trainer cues. A catalog name is recommended only when the leading score clears both a confidence threshold and an ambiguity margin. GPT-5.6 performs whole-session synthesis from the evidence plus those candidates, using only a recommended catalog name and otherwise preserving the original wording. It also answers questions over retrieved finalized workout records and rewrites contextual follow-up questions into standalone retrieval queries. Retrieval remains deterministic and grounded: authenticated SQL and pgvector select only the signed-in user’s finalized records, then GPT-5.6 generates an answer from that supplied context. Model output is treated as untrusted and reviewed workout corrections remain authoritative.
